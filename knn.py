...
class KNN(BinaryClassifier):
    ...
    def predict(self, X):
        ...
        if self.trY.size == 0:
            ...
            distanceArray = []
            for idx, x in enumerate(self.trX):
                temp = np.subtract(x, X)
                temp = np.square(temp)
                distance = math.sqrt(np.sum(temp))
                distanceArray.append((distance, idx))
                
            distanceArray = np.array(distanceArray, dtype=[('value', float), ('index', int)])
            distanceArray = np.sort(distanceArray, order='value')
            distanceArray = distanceArray[:K]
            
            finalRes = self.trY[distanceArray['index']]
            finalRes = np.sort(finalRes)[::-1]
            val = util.mode(finalRes)
            ...
            distanceArray = np.array(distanceArray, dtype=[('value', float), ('index', int)])
            distanceArray = np.sort(distanceArray, order='value')
            distanceArray = distanceArray[distanceArray['value'] <= eps]
